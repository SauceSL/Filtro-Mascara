...
def main():
    ...
    grid_size = 20
    step = 20
    grid_points = np.array([(x, y) for y in range(100, 400, step) for x in range(200, 600, step)], dtype=np.float32)
    ...
    while not glfw.window_should_close(window):
        ...
        # Dibujar la matriz de puntos y las líneas verdes
        for point in grid_points:
            x, y = map(int, point.ravel())
            cv.circle(frame, (x, y), 3, (0, 255, 0), -1)

        # Calcular escala, traslación y rotación
        translation = calculate_translation_and_rotation(mask, grid_points)
        scale = calculate_scale(mask, grid_points)

        # Verificar si toca el margen y resetear al centro si es necesario
        check_bounds()

        # Mostrar ventana OpenCV
        cv.imshow('Video', frame)

        # Renderizar escena OpenGL
        render()
